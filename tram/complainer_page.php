...
session_start();
    ...
    if(!$connection)
    {
        die("could not connect".$conction_error());
    }
    $db_select = mysqli_select_db($connection, "crime_portal");
    
    $u_id=$_SESSION['u_id'];
        
        $result=mysqli_query($connection,"SELECT a_no FROM user where u_id='$u_id' ");
        $q2=mysqli_fetch_assoc($result);
        $a_no=$q2['a_no'];
    
        $result1=mysqli_query($connection,"SELECT u_name FROM user where u_id='$u_id' ");
...
if(isset($_POST['s'])){
    ...
    if($var>=0)
    {
      ...
      mysqli_select_db($connection,"crime_portal"); 
			...
			<div class="login-form"><p><h2 style="color:white">Welcome <?php echo "$u_name" ?></h2></p><br>
				...
				<div class="top-w3-agile" style="color: gray">Type of Crime
					<select class="form-control" name="type_crime">
                        ...
                        <option>any other pls describe in the discription </option>
				    </select>
				</div>
