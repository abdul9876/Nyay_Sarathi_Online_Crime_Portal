...
<head>
    ...
    $connection=mysqli_connect("localhost","root","","crime_portal");
    if(!$connection)
    {
        die("could not connect".mysqli_error());
    }
    $db_select = mysqli_select_db($connection, "crime_portal");
    ...
    if(isset($_POST['status'])){
        ...
        {
            $upd=$_POST['update'];
            $qu1=mysqli_query($connection,"insert into update_case(c_id,case_update) values('$cid','$upd')");
        }
    }
    ...
    if(isset($_POST['close'])){
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            $qu2=mysqli_query($connection,"insert into update_case(c_id,case_update) values('$cid','$up')");
            $q2=mysqli_query($connection,"update complaint set pol_status='ChargeSheet Filed' where c_id='$cid'");
           
        }
    }
     $res2=mysqli_query($connection,"select d_o_u,case_update from update_case where c_id='$cid'");
...
?>
  ...
  <div style="width: 100%; height: 250px;"> 
   ...
   color: white;
   text-align: center;">
  <h4 style="color: white;"><b>NYAY SARATHI 2022</b></h4>
</div> 
