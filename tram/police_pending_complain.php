...
<head>
    ...
    if(isset($_POST['s2']))
    {
      if($_SERVER["REQUEST_METHOD"]=="POST")
      {
       $cid=$_POST['cid'];
       $_SESSION['cid']=$cid;
       $alok=mysqli_query($connection,"SELECT p_id FROM complaint WHERE c_id='$cid'");
    ...
    $p_id=$_SESSION['pol'];
     $query= "select c_id,type_crime,d_o_c,location FROM complaint where p_id='$p_id' and pol_status='In Process' order by c_id desc";
     $relf=mysqli_query($connection,$query);  
    ?>
 <script>
     function f1()
        {
...
<?php
      while($rows=mysqli_fetch_assoc($relf)){
    ?> 
